...
- [ ] 1.1 Add `coordinates: boolean` to `ClusterStatSettings` in `server/src/web/lib/settings.ts` and set it to `true` in `DEFAULT_CLUSTER_SETTINGS`.

## 2. Readout

- [ ] 2.1 In `server/src/web/components/ConditionCluster.tsx`, derive the displayed coordinates from the existing `map:position` subscription: `Math.floor` of `x` and `y`, and `z` only when it is not 0.
...
- [ ] 3.1 In `server/src/web/screens/SettingsScreen.tsx`, add a "Coordinates" entry to `CLUSTER_STATS` so it renders as a `SettingRow` disabled with the rest of the cluster, with a hint naming what it shows.
