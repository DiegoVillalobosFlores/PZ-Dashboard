...
- [ ] 1.2 Add both fields to `StatusSnapshot` in `server/src/web/lib/liveTypes.ts`.
...
- [ ] 2.1 Add `PZ_MEMORY_DB` to `server/src/config.ts`, defaulting to `join(APP_DIR, "data", "containers.sqlite")`, creating the directory if missing.
- [ ] 2.2 Create `server/src/state/memory.ts` opening the database with `bun:sqlite` and creating the two tables and their indexes on first use.
...
- [ ] 3.1 In `server/src/index.ts`, subscribe a second `onCategoryUpdate` listener that calls `recordSighting` when the `containers` category updates, reading the latest `status` snapshot from the store for the save id and game day.
...
- [ ] 5.1 Add a `useRememberedContainers` toggle to `server/src/web/lib/settings.ts`, defaulting to `false`, alongside `useFogOfWar`.
- [ ] 5.2 Add its row to `SettingsScreen` next to fog of war.
- [ ] 5.3 In `MapCanvas`, when the toggle is on, fetch remembered containers for the same padded bounding box already used for vector features, and refetch on the same condition.
- [ ] 5.4 Draw a marker per remembered container, with its name and last-seen day in the title, visible regardless of fog of war.

## 6. Search drawer

- [ ] 6.1 Add a memory search drawer modelled on `AnnotationsDrawer`, with a text field querying `/api/memory/containers?q=`, listing container name, remembered matching items, position and last-seen day.
- [ ] 6.2 Open it from a `HudIconButton` in `HudShell` next to the existing map-notes button.
