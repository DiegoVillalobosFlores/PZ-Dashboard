...
- [x] 1.1 In `PZDashboard_Collectors.lua`, extend the loop that clears
  `current` on every tracked vehicle so it also nils the telemetry fields, so a
  parked vehicle never carries stale readings
- [x] 1.2 On the entry marked `current`, collect the telemetry listed in
...
- [x] 1.3 Add `worstPartCondition` as a single pass over
  `getPartByIndex(0..getPartCount()-1)` reducing to the minimum condition, the
  whole loop inside one `safe()` call
- [x] 1.4 Add `vehicleInterval = 0.5` to the `vehicles` row in
...
- [x] 2.1 Add the telemetry fields to `VehicleSnapshot` in
  `packages/web/lib/liveTypes.ts`, all optional
- [x] 2.2 Add `useDrivingHud` (`pz-dashboard.drivingHud`, default `true`) and
...
- [x] 2.3 Add the driving HUD toggle to the map section of
...
- [x] 2.4 Extend `packages/web/lib/settings.test.ts` to cover the new defaults

## 3. Driving HUD component

- [x] 3.1 Create the HUD component subscribing under its own keys —
...
- [x] 3.2 Render the readouts: vehicle name, speed with unit, fuel, and engine,
  headlight, battery and damage indicators, each showing "unavailable" when its
  field is absent rather than rendering a zero
- [x] 3.3 Emphasise the fuel readout below the level the game itself warns at,
  and dim all readings once the arrival stamp is older than several times the
  expected interval
- [x] 3.4 Add the collapse control: collapsed shows speed only, expanded shows
  everything, state read from and written to the persisted setting
- [x] 3.5 Return nothing when the setting is off or the player is not in a
  vehicle
- [x] 3.6 Add a unit test for the pure parts — deriving the current vehicle
  from a `vehicles` snapshot, the low-fuel threshold, and the staleness cutoff

## 4. Shell integration and layout

- [x] 4.1 Mount the HUD in `HudShell.tsx` so it survives navigation between
  screens
- [x] 4.2 Position it with the shell's `--hud-*-inset` and
...
- [x] 4.3 Confine `pointerEvents` to the HUD's own box so map pan and zoom
  outside it are unaffected

## 5. Verification

- [x] 5.1 Run the web test suite and typecheck
