# browser-direct-mode Specification
...
be reachable at the same paths in the browser app, so that the shared
frontend runs unmodified against either. Responses SHALL be equivalent in
content type and status semantics, including the not-found and bad-request
cases.
