...
function parseWorldMap(path: string): { features: PolygonFeature[]; places: PlaceLabel[] } {
...
function parseStreets(path: string): StreetLabel[] {
  const xml = readFileSync(path, "utf8");
...
export function getVectorMap(region: string): VectorMapData {
  ...
  const dir = join(PZ_INSTALL_DIR, "media", "maps", region);
  ...
  const { features, places } = parseWorldMap(worldMapPath);
  const streets = existsSync(streetsPath) ? parseStreets(streetsPath) : [];
...
export function queryVectorMap(
  region: string,
  x1: number,
  y1: number,
  x2: number,
  ...
  const data = getVectorMap(region);
  return {
