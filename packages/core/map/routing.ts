...
async function getGraph(files: GameFiles, installDir: string, region: string): Promise<RouteGraph> {
  let graph = graphCache.get(region);
  if (!graph) {
    graph = buildGraph((await getVectorMap(files, region, installDir)).streets);
    graphCache.set(region, graph);
  }
  return graph;
}

export async function findRoute(files: GameFiles, installDir: string, region: string, from: RoutePoint, to: RoutePoint): Promise<RouteResult | null> {
  const graph = await getGraph(files, installDir, region);
