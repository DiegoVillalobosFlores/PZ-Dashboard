...
async function clothingDef(files: GameFiles, installDir: string, name: string): Promise<ClothingDef | null> {
  const cached = clothingCache.get(name);
  if (cached !== undefined) return cached;

  let def: ClothingDef | null = null;
  const path = await resolveMediaPath(files, `clothing/clothingItems/${name}.xml`, installDir);
  if (path) {
    try {
      const xml = new TextDecoder().decode(await files.read(path));
...
async function loadHairStyles(files: GameFiles, installDir: string): Promise<Map<string, Style>> {
  if (hairStyles) return hairStyles;
  const styles = new Map<string, Style>();
  const path = await resolveMediaPath(files, "hairStyles/hairStyles.xml", installDir);
  if (path) {
    const xml = await files.read(path).then((bytes) => new TextDecoder().decode(bytes)).catch(() => "");
...
async function loadBeardStyles(files: GameFiles, installDir: string): Promise<Map<string, Style>> {
  if (beardStyles) return beardStyles;
  const styles = new Map<string, Style>();
  // Not a typo: beardStyles.xml sits in the hairStyles folder alongside it.
  const path = await resolveMediaPath(files, "hairStyles/beardStyles.xml", installDir);
  if (path) {
    const xml = await files.read(path).then((bytes) => new TextDecoder().decode(bytes)).catch(() => "");
...
async function resolveBodyTexture(files: GameFiles, installDir: string, name: string): Promise<string | null> {
  return (await resolveTexturePath(files, `Body/${name}`, installDir)) ? `Body/${name}` : (await resolveTexturePath(files, name, installDir)) ? name : null;
}
...
async function loadBodySkeleton(files: GameFiles, installDir: string, female: boolean): Promise<Map<string, number[]> | null> {
  const key = female ? 'f' : 'm';
  const cached = bodySkeletons.get(key);
  if (cached) return cached;
  const bodyModel = female ? "skinned/FemaleBody" : "skinned/MaleBody";
  const absPath = await resolveModelPath(files, bodyModel, installDir);
  if (!absPath) return null;
  const mesh = await loadMesh(files.read, absPath);
  if (!mesh) return null;
...
export async function buildFigure(files: GameFiles, installDir: string, appearance: Appearance): Promise<Figure> {
  const female = Boolean(appearance.female);
  const parts: FigurePart[] = [];
  const missing: string[] = [];

  const bodyModel = female ? "skinned/FemaleBody" : "skinned/MaleBody";
  const skeleton = await loadBodySkeleton(files, installDir, female);
  const bodyPath = await resolveModelPath(files, bodyModel, installDir);
  if (bodyPath) {
    const skin = await resolveBodyTexture(files, installDir, bodyTexture(appearance, female));
  ...
  for (const [index, worn] of (appearance.worn ?? []).entries()) {
    ...
    const def = await clothingDef(files, installDir, itemName);
    if (!def) {
    ...
    if (hasModel && !(await resolveModelPath(files, modelPath, installDir))) {
    ...
    parts.push({
      ...
      texture: texture && (await resolveTexturePath(files, texture, installDir)) ? textureUrl(texture) : null,
      tint: rgb(worn.tint),
  ...
  if (hairName) {
    ...
    const styles = await loadHairStyles(files, installDir);
    ...
    if (style?.model && (await resolveModelPath(files, style.model, installDir))) {
      parts.push({
        id: "hair",
        label: `Hair (${hairName})`,
        model: modelUrl(style.model),
        texture: style.texture && (await resolveTexturePath(files, style.texture, installDir)) ? textureUrl(style.texture) : null,
  ...
  if (beardName) {
    const style = (await loadBeardStyles(files, installDir)).get(beardName.toLowerCase());
    if (style?.model && (await resolveModelPath(files, style.model, installDir))) {
      parts.push({
        id: "beard",
